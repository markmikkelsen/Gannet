...
if MRS_struct.p.TE(ii) < 100
    ...
    % Asp amplitudes are allowed either sign as the shape of the Asp
    % multiplet varies between sequences (e.g., MEGA-PRESS vs. sLASER)
    lb = [ 0               -2000  2.95-0.02 ...
          -2*maxinGSH      -5000            ...
           0               -5000  2.71-0.02 ...
          -2*abs(maxinAsp) -5000            ...
          -2*abs(maxinAsp) -5000  2.50      ...
          -2*abs(maxinAsp) -5000            ...
          -2*abs(maxinAsp) -2000  2.40      ...
          -2*maxinGSH      -2000  2.36-0.02];

    ub = [ 2*maxinGSH      -125  2.95+0.02 ...
           2*maxinGSH      -300            ...
           3*maxinGSH      -300  2.71+0.02 ...
           2*abs(maxinAsp) -125            ...
           2*abs(maxinAsp) -125  2.62      ...
           2*abs(maxinAsp) -125            ...
           2*abs(maxinAsp) -300  2.50      ...
           0               -125  2.36+0.02];
...
% The sign of the Asp signal depends on the sequence and TE, so amplitude
% bounds derived from maxinAsp can come out reversed; reorder them so that
% lb <= ub and keep the initial values within bounds
swap = false(size(lb));
swap(amplParams) = lb(amplParams) > ub(amplParams);
[lb(swap), ub(swap)] = deal(ub(swap), lb(swap));
modelParamInit = min(max(modelParamInit, lb), ub);
